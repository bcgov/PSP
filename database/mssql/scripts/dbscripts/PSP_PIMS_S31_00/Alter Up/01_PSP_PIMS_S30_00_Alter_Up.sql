-- Script generated by Aqua Data Studio Schema Synchronization for MS SQL Server 2016 on Thu Jun 23 11:45:06 PDT 2022
...
-- Create table dbo.PIMS_DOCUMENT_TYPE
PRINT N'Create table dbo.PIMS_DOCUMENT_TYPE'
GO
CREATE TABLE [dbo].[PIMS_DOCUMENT_TYPE]  ( 
	[DOCUMENT_TYPE_ID]              	bigint NOT NULL CONSTRAINT [DOCTYP_DOCUMENT_TYPE_ID_DEF]  DEFAULT (NEXT VALUE FOR [PIMS_DOCUMENT_TYPE_ID_SEQ]),
	[MAYAN_ID]                      	bigint NOT NULL CONSTRAINT [DOCTYP_MAYAN_ID_DEF]  DEFAULT ('<Empty>'),
...
EXEC sp_addextendedproperty 
	...
	@level0type = N'Schema', @level0name = N'dbo', 
	@level1type = N'Table', @level1name = N'PIMS_DOCUMENT_TYPE', 
...
CREATE TABLE [dbo].[PIMS_DOCUMENT]  ( 
	...
	[MAYAN_ID]                      	bigint NOT NULL CONSTRAINT [DOCMNT_MAYAN_ID_DEF]  DEFAULT ('<Empty>'),
...
CREATE TABLE [dbo].[PIMS_NOTE]  ( 
	...
	[NOTE_TXT]                      	nvarchar(max) NOT NULL CONSTRAINT [NOTE_NOTE_TXT_DEF]  DEFAULT ('<Empty>'),
...
ALTER TABLE [dbo].[PIMS_DOCUMENT]
	...
	REFERENCES [dbo].[PIMS_DOCUMENT_TYPE]([DOCUMENT_TYPE_ID])
	ON DELETE NO ACTION 
	ON UPDATE NO ACTION 
GO
IF @@ERROR <> 0 SET NOEXEC ON
...
GO
CREATE TRIGGER [dbo].[PIMS_DOCTYP_I_S_U_TR] ON PIMS_DOCUMENT_TYPE INSTEAD OF UPDATE AS
SET NOCOUNT ON
BEGIN TRY
  IF NOT EXISTS(SELECT * FROM deleted) 
    RETURN;
  ...
  update PIMS_DOCUMENT_TYPE
    ...
    , DB_LAST_UPDATE_TIMESTAMP = getutcdate()
    , DB_LAST_UPDATE_USERID = user_name()
    from PIMS_DOCUMENT_TYPE
    inner join inserted
    on (PIMS_DOCUMENT_TYPE.DOCUMENT_TYPE_ID = inserted.DOCUMENT_TYPE_ID);
...
GO
CREATE TRIGGER [dbo].[PIMS_DOCTYP_I_S_I_TR] ON PIMS_DOCUMENT_TYPE INSTEAD OF INSERT AS
SET NOCOUNT ON
BEGIN TRY
  IF NOT EXISTS(SELECT * FROM inserted) 
    RETURN;
  ...
  insert into PIMS_DOCUMENT_TYPE ("DOCUMENT_TYPE_ID",
      "MAYAN_ID",
      "DOCUMENT_TYPE",
      "CONCURRENCY_CONTROL_NUMBER",
      "APP_CREATE_TIMESTAMP",
