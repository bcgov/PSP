...
FROM   SYS.SEQUENCES
WHERE  NAME = 'PIMS_MANAGEMENT_ACTIVITY_ID_SEQ';

SET @qry = N'ALTER SEQUENCE dbo.PIMS_PROPERTY_ACTIVITY_ID_SEQ RESTART WITH ' + CONVERT(NVARCHAR, @CurrVlu);
...
SELECT @CurrVlu = CONVERT(BIGINT, current_value) + 1
FROM   SYS.SEQUENCES
WHERE  NAME = 'PIMS_MANAGEMENT_ACTIVITY_PROPERTY_ID_SEQ';

SET @qry = N'ALTER SEQUENCE dbo.PIMS_PROP_PROP_ACTIVITY_ID_SEQ RESTART WITH ' + CONVERT(NVARCHAR, @CurrVlu);
EXEC sp_executesql @qry;
GO
IF @@ERROR <> 0 SET NOEXEC ON
GO
