-- Script generated by Aqua Data Studio Schema Synchronization for MS SQL Server 2016 on Mon Dec 23 11:37:02 PST 2024
...
-- Create table dbo.PIMS_PROJECT_TEAM
PRINT N'Create table dbo.PIMS_PROJECT_TEAM'
GO
CREATE TABLE [dbo].[PIMS_PROJECT_TEAM]  ( 
	[PROJECT_TEAM_ID]               	bigint NOT NULL CONSTRAINT [PROJTM_PROJECT_TEAM_ID_DEF]  DEFAULT (NEXT VALUE FOR [PIMS_PROJECT_TEAM_ID_SEQ]),
	[PROJECT_ID]                    	bigint NULL,
	[PERSON_ID]                     	bigint NULL,
	[CONCURRENCY_CONTROL_NUMBER]    	bigint NOT NULL CONSTRAINT [PROJTM_CONCURRENCY_CONTROL_NUMBER_DEF]  DEFAULT ((1)),
	[APP_CREATE_TIMESTAMP]          	datetime NOT NULL CONSTRAINT [PROJTM_APP_CREATE_TIMESTAMP_DEF]  DEFAULT (getutcdate()),
	[APP_CREATE_USERID]             	nvarchar(30) NOT NULL CONSTRAINT [PROJTM_APP_CREATE_USERID_DEF]  DEFAULT (user_name()),
	[APP_CREATE_USER_GUID]          	uniqueidentifier NULL,
	[APP_CREATE_USER_DIRECTORY]     	nvarchar(30) NOT NULL CONSTRAINT [PROJTM_APP_CREATE_USER_DIRECTORY_DEF]  DEFAULT (user_name()),
	[APP_LAST_UPDATE_TIMESTAMP]     	datetime NOT NULL CONSTRAINT [PROJTM_APP_LAST_UPDATE_TIMESTAMP_DEF]  DEFAULT (getutcdate()),
	[APP_LAST_UPDATE_USERID]        	nvarchar(30) NOT NULL CONSTRAINT [PROJTM_APP_LAST_UPDATE_USERID_DEF]  DEFAULT (user_name()),
	[APP_LAST_UPDATE_USER_GUID]     	uniqueidentifier NULL,
	[APP_LAST_UPDATE_USER_DIRECTORY]	nvarchar(30) NOT NULL CONSTRAINT [PROJTM_APP_LAST_UPDATE_USER_DIRECTORY_DEF]  DEFAULT (user_name()),
	[DB_CREATE_TIMESTAMP]           	datetime NOT NULL CONSTRAINT [PROJTM_DB_CREATE_TIMESTAMP_DEF]  DEFAULT (getutcdate()),
	[DB_CREATE_USERID]              	nvarchar(30) NOT NULL CONSTRAINT [PROJTM_DB_CREATE_USERID_DEF]  DEFAULT (user_name()),
	[DB_LAST_UPDATE_TIMESTAMP]      	datetime NOT NULL CONSTRAINT [PROJTM_DB_LAST_UPDATE_TIMESTAMP_DEF]  DEFAULT (getutcdate()),
	[DB_LAST_UPDATE_USERID]         	nvarchar(30) NOT NULL CONSTRAINT [PROJTM_DB_LAST_UPDATE_USERID_DEF]  DEFAULT (user_name()),
	CONSTRAINT [PROJTM_PK] PRIMARY KEY CLUSTERED([PROJECT_TEAM_ID])
 ON [PRIMARY])
GO
EXEC sp_addextendedproperty 
	@name = N'MS_Description', @value = N'Generated surrogate primary key.' , 
	@level0type = N'Schema', @level0name = N'dbo', 
	@level1type = N'Table', @level1name = N'PIMS_PROJECT_TEAM', 
	@level2type = N'Column', @level2name = N'PROJECT_TEAM_ID'
GO
EXEC sp_addextendedproperty 
...
GO
EXEC sp_addextendedproperty 
	@name = N'MS_Description', @value = N'Table to support multiple persons associated with a project team.' , 
	@level0type = N'Schema', @level0name = N'dbo', 
	@level1type = N'Table', @level1name = N'PIMS_PROJECT_TEAM'
GO
IF @@ERROR <> 0 SET NOEXEC ON
...
-- Create sequence dbo.PIMS_PROJECT_TEAM_H_ID_SEQ
PRINT N'Create sequence dbo.PIMS_PROJECT_TEAM_H_ID_SEQ'
GO
CREATE SEQUENCE [dbo].[PIMS_PROJECT_TEAM_H_ID_SEQ]
	AS bigint
	START WITH 1
...
-- Create table dbo.PIMS_PROJECT_TEAM_HIST
PRINT N'Create table dbo.PIMS_PROJECT_TEAM_HIST'
GO
CREATE TABLE [dbo].[PIMS_PROJECT_TEAM_HIST]  ( 
	[_PROJECT_TEAM_HIST_ID]         	bigint NOT NULL DEFAULT (NEXT VALUE FOR [PIMS_PROJECT_TEAM_H_ID_SEQ]),
	...
	CONSTRAINT [PIMS_PROJTM_H_PK] PRIMARY KEY CLUSTERED([_PROJECT_TEAM_HIST_ID])
 ON [PRIMARY])
GO
IF @@ERROR <> 0 SET NOEXEC ON
GO

-- Create unique constraint dbo.PIMS_PROJTM_H_UK
PRINT N'Create unique constraint dbo.PIMS_PROJTM_H_UK'
GO
ALTER TABLE [dbo].[PIMS_PROJECT_TEAM_HIST]
	ADD CONSTRAINT [PIMS_PROJTM_H_UK]
	UNIQUE ([_PROJECT_TEAM_HIST_ID], [END_DATE_HIST]) 
	WITH (
...
-- Create index dbo.PROJTM_PROJECT_ID_IDX
PRINT N'Create index dbo.PROJTM_PROJECT_ID_IDX'
GO
CREATE NONCLUSTERED INDEX [PROJTM_PROJECT_ID_IDX]
	ON [dbo].[PIMS_PROJECT_TEAM]([PROJECT_ID])
GO
IF @@ERROR <> 0 SET NOEXEC ON
GO
...
-- Create index dbo.PROJTM_PERSON_ID_IDX
PRINT N'Create index dbo.PROJTM_PERSON_ID_IDX'
GO
CREATE NONCLUSTERED INDEX [PROJTM_PERSON_ID_IDX]
	ON [dbo].[PIMS_PROJECT_TEAM]([PERSON_ID])
GO
IF @@ERROR <> 0 SET NOEXEC ON
GO
...
-- Create foreign key constraint dbo.PIM_PERSON_PIM_PROJTM_FK
PRINT N'Create foreign key constraint dbo.PIM_PERSON_PIM_PROJTM_FK'
GO
ALTER TABLE [dbo].[PIMS_PROJECT_TEAM]
	ADD CONSTRAINT [PIM_PERSON_PIM_PROJTM_FK]
...
-- Create foreign key constraint dbo.PIM_PROJCT_PIM_PROJTM_FK
PRINT N'Create foreign key constraint dbo.PIM_PROJCT_PIM_PROJTM_FK'
GO
ALTER TABLE [dbo].[PIMS_PROJECT_TEAM]
	ADD CONSTRAINT [PIM_PROJCT_PIM_PROJTM_FK]
...
-- Create trigger dbo.PIMS_PROJTM_I_S_U_TR
PRINT N'Create trigger dbo.PIMS_PROJTM_I_S_U_TR'
GO
CREATE TRIGGER [dbo].[PIMS_PROJTM_I_S_U_TR] ON PIMS_PROJECT_TEAM INSTEAD OF UPDATE AS
SET NOCOUNT ON
BEGIN TRY
  ...
  if exists (select 1 from inserted, deleted where inserted.CONCURRENCY_CONTROL_NUMBER != deleted.CONCURRENCY_CONTROL_NUMBER+1 AND inserted.PROJECT_TEAM_ID = deleted.PROJECT_TEAM_ID)
    raiserror('CONCURRENCY FAILURE.',16,1)


  -- update statement
  update PIMS_PROJECT_TEAM
    set "PROJECT_TEAM_ID" = inserted."PROJECT_TEAM_ID",
    ...
    from PIMS_PROJECT_TEAM
    inner join inserted
    on (PIMS_PROJECT_TEAM.PROJECT_TEAM_ID = inserted.PROJECT_TEAM_ID);
...
-- Create trigger dbo.PIMS_PROJTM_I_S_I_TR
PRINT N'Create trigger dbo.PIMS_PROJTM_I_S_I_TR'
GO
CREATE TRIGGER [dbo].[PIMS_PROJTM_I_S_I_TR] ON PIMS_PROJECT_TEAM INSTEAD OF INSERT AS
SET NOCOUNT ON
BEGIN TRY
  IF NOT EXISTS(SELECT * FROM inserted) 
    RETURN;
  ...
  insert into PIMS_PROJECT_TEAM ("PROJECT_TEAM_ID",
      "PROJECT_ID",
      "PERSON_ID",
      "CONCURRENCY_CONTROL_NUMBER",
      "APP_CREATE_TIMESTAMP",
...
-- Create trigger dbo.PIMS_PROJTM_A_S_IUD_TR
PRINT N'Create trigger dbo.PIMS_PROJTM_A_S_IUD_TR'
GO
CREATE TRIGGER [dbo].[PIMS_PROJTM_A_S_IUD_TR] ON PIMS_PROJECT_TEAM FOR INSERT, UPDATE, DELETE AS
SET NOCOUNT ON
BEGIN TRY
DECLARE @curr_date datetime;
SET @curr_date = getutcdate();
  ...
  -- historical
  IF EXISTS(SELECT * FROM deleted)
    update PIMS_PROJECT_TEAM_HIST set END_DATE_HIST = @curr_date where PROJECT_TEAM_ID in (select PROJECT_TEAM_ID from deleted) and END_DATE_HIST is null;
  
  IF EXISTS(SELECT * FROM inserted)
    insert into PIMS_PROJECT_TEAM_HIST ([PROJECT_TEAM_ID], [PROJECT_ID], [PERSON_ID], [CONCURRENCY_CONTROL_NUMBER], [APP_CREATE_TIMESTAMP], [APP_CREATE_USERID], [APP_CREATE_USER_GUID], [APP_CREATE_USER_DIRECTORY], [APP_LAST_UPDATE_TIMESTAMP], [APP_LAST_UPDATE_USERID], [APP_LAST_UPDATE_USER_GUID], [APP_LAST_UPDATE_USER_DIRECTORY], [DB_CREATE_TIMESTAMP], [DB_CREATE_USERID], [DB_LAST_UPDATE_TIMESTAMP], [DB_LAST_UPDATE_USERID], _PROJECT_TEAM_HIST_ID, END_DATE_HIST, EFFECTIVE_DATE_HIST)
      select [PROJECT_TEAM_ID], [PROJECT_ID], [PERSON_ID], [CONCURRENCY_CONTROL_NUMBER], [APP_CREATE_TIMESTAMP], [APP_CREATE_USERID], [APP_CREATE_USER_GUID], [APP_CREATE_USER_DIRECTORY], [APP_LAST_UPDATE_TIMESTAMP], [APP_LAST_UPDATE_USERID], [APP_LAST_UPDATE_USER_GUID], [APP_LAST_UPDATE_USER_DIRECTORY], [DB_CREATE_TIMESTAMP], [DB_CREATE_USERID], [DB_LAST_UPDATE_TIMESTAMP], [DB_LAST_UPDATE_USERID], (next value for [dbo].[PIMS_PROJECT_TEAM_H_ID_SEQ]) as [_PROJECT_TEAM_HIST_ID], null as [END_DATE_HIST], @curr_date as [EFFECTIVE_DATE_HIST] from inserted;
