-- Script generated by Aqua Data Studio Schema Synchronization for MS SQL Server 2016 on Wed Apr 03 11:57:20 PDT 2024
...
GO
ALTER TABLE [dbo].[PIMS_PROPERTY]
	DROP CONSTRAINT IF EXISTS [PRPRTY_IS_DISPOSED_DEF]
GO
IF @@ERROR <> 0 SET NOEXEC ON
GO
ALTER TABLE [dbo].[PIMS_PROPERTY]
...
GO
ALTER TABLE [dbo].[PIMS_PROPERTY]
	DROP CONSTRAINT IF EXISTS [PRPRTY_IS_PROPERTY_OF_INTEREST_DEF]
GO
IF @@ERROR <> 0 SET NOEXEC ON
GO
ALTER TABLE [dbo].[PIMS_PROPERTY]
	DROP COLUMN IF EXISTS [IS_DISPOSED], [IS_OTHER_INTEREST], [IS_PROPERTY_OF_INTEREST]
GO
IF @@ERROR <> 0 SET NOEXEC ON
GO
...
GO
ALTER TABLE [dbo].[PIMS_PROPERTY_HIST] ALTER COLUMN [IS_DISPOSED] bit NULL
GO
IF @@ERROR <> 0 SET NOEXEC ON
GO
ALTER TABLE [dbo].[PIMS_PROPERTY_HIST] ALTER COLUMN [IS_OTHER_INTEREST] bit NULL
GO
IF @@ERROR <> 0 SET NOEXEC ON
GO
ALTER TABLE [dbo].[PIMS_PROPERTY_HIST] ALTER COLUMN [IS_PROPERTY_OF_INTEREST] bit NULL
GO
IF @@ERROR <> 0 SET NOEXEC ON
GO
...
SELECT PROP.PROPERTY_ID
     ...
     , IIF(EXISTS (SELECT 1
                   ...
                   WHERE  DISP.DISPOSITION_FILE_STATUS_TYPE_CODE <> N'CANCELLED'
                      AND DISP.COMPLETED_DT                      IS NOT NULL), CONVERT([bit],(1)), CONVERT([bit],(0))) AS IS_DISPOSED
     , CASE
         WHEN EXISTS (SELECT 1
                      ...
                      WHERE  TAKE.IS_NEW_LAND_ACT       = 1
                         ...
                         AND TAKE.LAND_ACT_TYPE_CODE   IN (N'Section 15', N'Section 15', N'Section 17', N'Section 66', N'NOI')) THEN CONVERT([bit],(1))
         WHEN EXISTS (SELECT 1
                      FROM   PIMS_TAKE                      TAKE JOIN
                             PIMS_PROPERTY_ACQUISITION_FILE PRAF   ON PRAF.PROPERTY_ACQUISITION_FILE_ID = TAKE.PROPERTY_ACQUISITION_FILE_ID
                                                                  AND PRAF.PROPERTY_ID                  = PROP.PROPERTY_ID
         ...
         WHEN EXISTS (SELECT 1
                      FROM   PIMS_TAKE                      TAKE                                                                            JOIN
                             PIMS_PROPERTY_ACQUISITION_FILE PRAF   ON PRAF.PROPERTY_ACQUISITION_FILE_ID = TAKE.PROPERTY_ACQUISITION_FILE_ID
                                                                  AND PRAF.PROPERTY_ID                  = PROP.PROPERTY_ID                  JOIN
                             PIMS_PROPERTY_LEASE            PRLS   ON PRLS.PROPERTY_ID                  = PROP.PROPERTY_ID                  JOIN
                             PIMS_LEASE                     LEAS   ON LEAS.LEASE_ID                     = PRLS.LEASE_ID
     ...
     , IIF(EXISTS (SELECT 1
                   FROM   PIMS_PROPERTY_ACQUISITION_FILE PRAF JOIN
                          PIMS_ACQUISITION_FILE          ACQF   ON ACQF.ACQUISITION_FILE_ID = PRAF.ACQUISITION_FILE_ID
                                                               AND PRAF.PROPERTY_ID         = PROP.PROPERTY_ID
                   WHERE  ACQF.COMPLETION_DATE                   IS NOT NULL
                      AND ACQF.ACQUISITION_FILE_STATUS_TYPE_CODE <> N'CANCEL'), CONVERT([bit],(1)), CONVERT([bit],(0))) AS HAS_ACTIVE_ACQUISITION_FILE
     , IIF(EXISTS (SELECT 1
                   ...
                   WHERE  RSHF.RESEARCH_COMPLETION_DATE       IS NOT NULL
                      AND RSHF.RESEARCH_FILE_STATUS_TYPE_CODE <> N'INACTIVE'), CONVERT([bit],(1)), CONVERT([bit],(0)))  AS HAS_ACTIVE_RESEARCH_FILE
     , IIF(EXISTS (SELECT 1
...
SET @curr_date = getutcdate();
  ...
  IF EXISTS(SELECT * FROM inserted)
    insert into PIMS_TAKE_HIST ([TAKE_ID], [PROPERTY_ACQUISITION_FILE_ID], [TAKE_TYPE_CODE], [TAKE_STATUS_TYPE_CODE], [TAKE_SITE_CONTAM_TYPE_CODE], [AREA_UNIT_TYPE_CODE], [LAND_ACT_TYPE_CODE], [DESCRIPTION], [COMPLETION_DT], [IS_NEW_HIGHWAY_DEDICATION], [NEW_HIGHWAY_DEDICATION_AREA], [IS_ACQUIRED_FOR_INVENTORY], [IS_NEW_INTEREST_IN_SRW], [STATUTORY_RIGHT_OF_WAY_AREA], [SRW_END_DT], [IS_NEW_LICENSE_TO_CONSTRUCT], [LICENSE_TO_CONSTRUCT_AREA], [LTC_END_DT], [IS_NEW_LAND_ACT], [LAND_ACT_AREA], [LAND_ACT_END_DT], [IS_THERE_SURPLUS], [SURPLUS_AREA], [CONCURRENCY_CONTROL_NUMBER], [APP_CREATE_TIMESTAMP], [APP_CREATE_USERID], [APP_CREATE_USER_GUID], [APP_CREATE_USER_DIRECTORY], [APP_LAST_UPDATE_TIMESTAMP], [APP_LAST_UPDATE_USERID], [APP_LAST_UPDATE_USER_GUID], [APP_LAST_UPDATE_USER_DIRECTORY], [DB_CREATE_TIMESTAMP], [DB_CREATE_USERID], [DB_LAST_UPDATE_TIMESTAMP], [DB_LAST_UPDATE_USERID], _TAKE_HIST_ID, END_DATE_HIST, EFFECTIVE_DATE_HIST)
...
SET @defName = (SELECT obj.NAME
                ...
                WHERE  obj.XTYPE = 'D'
                   AND tbl.NAME = 'PIMS_PROPERTY_HIST' 
                   AND col.NAME = 'IS_PROPERTY_OF_INTEREST')
SET @sqlQry = 'ALTER TABLE [dbo].[PIMS_PROPERTY_HIST] DROP CONSTRAINT IF EXISTS [' + @defName + ']'
EXEC (@sqlQry)
GO
IF @@ERROR <> 0 SET NOEXEC ON
