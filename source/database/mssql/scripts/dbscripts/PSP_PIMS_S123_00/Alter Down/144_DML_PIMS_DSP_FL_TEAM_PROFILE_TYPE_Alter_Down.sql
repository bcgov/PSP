...
IF @@ROWCOUNT = 1
    ...
    WHERE  DSP_FL_TEAM_PROFILE_TYPE_CODE = @CurrCd;
ELSE
    INSERT  INTO PIMS_DSP_FL_TEAM_PROFILE_TYPE (
...
FROM   PIMS_DSP_FL_TEAM_PROFILE_TYPE AS biz
       INNER JOIN
       (SELECT DSP_FL_TEAM_PROFILE_TYPE_CODE,
               ROW_NUMBER() OVER (ORDER BY DESCRIPTION) AS ROW_NUM
        FROM   PIMS_DSP_FL_TEAM_PROFILE_TYPE) AS seq
       ON seq.DSP_FL_TEAM_PROFILE_TYPE_CODE = biz.DSP_FL_TEAM_PROFILE_TYPE_CODE
