...
IF @@ROWCOUNT = 1
    ...
    WHERE  ACQ_FL_TEAM_PROFILE_TYPE_CODE = @CurrCd;
ELSE
    INSERT  INTO PIMS_ACQ_FL_TEAM_PROFILE_TYPE (
...
FROM   PIMS_ACQ_FL_TEAM_PROFILE_TYPE AS biz
       INNER JOIN
       (SELECT ACQ_FL_TEAM_PROFILE_TYPE_CODE,
               ROW_NUMBER() OVER (ORDER BY DESCRIPTION) AS ROW_NUM
        FROM   PIMS_ACQ_FL_TEAM_PROFILE_TYPE) AS seq
       ON seq.ACQ_FL_TEAM_PROFILE_TYPE_CODE = biz.ACQ_FL_TEAM_PROFILE_TYPE_CODE
