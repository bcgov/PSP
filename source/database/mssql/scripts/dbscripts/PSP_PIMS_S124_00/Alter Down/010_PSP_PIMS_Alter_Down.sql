...
-- Restore the notification insert trigger without LEASE_PERIOD_ID.
ALTER TRIGGER [dbo].[PIMS_NOTIFY_I_S_I_TR] ON [dbo].[PIMS_NOTIFICATION] INSTEAD OF INSERT AS
SET NOCOUNT ON
BEGIN TRY
  IF NOT EXISTS(SELECT * FROM inserted)
    RETURN;

  INSERT INTO PIMS_NOTIFICATION ("NOTIFICATION_ID", "NOTIFICATION_TYPE_CODE", "ACQUISITION_FILE_ID", "DISPOSITION_FILE_ID", "RESEARCH_FILE_ID", "MANAGEMENT_FILE_ID", "LEASE_ID", "TAKE_ID", "INSURANCE_ID", "LEASE_CONSULTATION_ID", "NOTICE_OF_CLAIM_ID", "LEASE_RENEWAL_ID", "EXPROP_OWNER_HISTORY_ID", "AGREEMENT_ID", "NOTIFICATION_TRIGGER_DATE", "NOTIFICATION_MESSAGE", "CONCURRENCY_CONTROL_NUMBER", "APP_CREATE_TIMESTAMP", "APP_CREATE_USERID", "APP_CREATE_USER_GUID", "APP_CREATE_USER_DIRECTORY", "APP_LAST_UPDATE_TIMESTAMP", "APP_LAST_UPDATE_USERID", "APP_LAST_UPDATE_USER_GUID", "APP_LAST_UPDATE_USER_DIRECTORY")
...
-- Restore the notification update trigger without LEASE_PERIOD_ID.
ALTER TRIGGER [dbo].[PIMS_NOTIFY_I_S_U_TR] ON [dbo].[PIMS_NOTIFICATION] INSTEAD OF UPDATE AS
SET NOCOUNT ON
BEGIN TRY
  ...
  INNER JOIN inserted ON PIMS_NOTIFICATION.NOTIFICATION_ID = inserted.NOTIFICATION_ID;
END TRY
BEGIN CATCH
  IF @@TRANCOUNT > 0 ROLLBACK TRANSACTION
  EXEC pims_error_handling
...
-- Restore the notification history trigger without LEASE_PERIOD_ID.
ALTER TRIGGER [dbo].[PIMS_NOTIFY_A_S_IUD_TR] ON [dbo].[PIMS_NOTIFICATION] FOR INSERT, UPDATE, DELETE AS
SET NOCOUNT ON
BEGIN TRY
  ...
  IF EXISTS(SELECT * FROM inserted)
    ...
    SELECT [NOTIFICATION_ID], [NOTIFICATION_TYPE_CODE], [ACQUISITION_FILE_ID], [DISPOSITION_FILE_ID], [RESEARCH_FILE_ID], [MANAGEMENT_FILE_ID], [LEASE_ID], [TAKE_ID], [INSURANCE_ID], [LEASE_CONSULTATION_ID], [NOTICE_OF_CLAIM_ID], [LEASE_RENEWAL_ID], [EXPROP_OWNER_HISTORY_ID], [AGREEMENT_ID], [NOTIFICATION_TRIGGER_DATE], [NOTIFICATION_MESSAGE], [CONCURRENCY_CONTROL_NUMBER], [APP_CREATE_TIMESTAMP], [APP_CREATE_USERID], [APP_CREATE_USER_GUID], [APP_CREATE_USER_DIRECTORY], [APP_LAST_UPDATE_TIMESTAMP], [APP_LAST_UPDATE_USERID], [APP_LAST_UPDATE_USER_GUID], [APP_LAST_UPDATE_USER_DIRECTORY], [DB_CREATE_TIMESTAMP], [DB_CREATE_USERID], [DB_LAST_UPDATE_TIMESTAMP], [DB_LAST_UPDATE_USERID], NEXT VALUE FOR dbo.PIMS_NOTIFICATION_H_ID_SEQ, NULL, @curr_date
    FROM inserted;
END TRY
BEGIN CATCH
  IF @@TRANCOUNT > 0 ROLLBACK TRANSACTION
...
-- Remove objects added for LEASE_PERIOD_ID.
DROP INDEX IF EXISTS [NOTIFY_LEASE_PERIOD_ID_IDX] ON [dbo].[PIMS_NOTIFICATION]
GO
IF @@ERROR <> 0 SET NOEXEC ON
GO

ALTER TABLE [dbo].[PIMS_NOTIFICATION]
  DROP CONSTRAINT IF EXISTS [PIM_LSPERD_PIM_NOTIFY_FK]
GO
IF @@ERROR <> 0 SET NOEXEC ON
GO

ALTER TABLE [dbo].[PIMS_NOTIFICATION]
  DROP COLUMN IF EXISTS [LEASE_PERIOD_ID]
GO
IF @@ERROR <> 0 SET NOEXEC ON
GO
...
GO
ALTER TABLE [dbo].[PIMS_ACQUISITION_FILE]
	DROP COLUMN IF EXISTS [OWNER_REP_COMMENT]
GO
IF @@ERROR <> 0 SET NOEXEC ON
GO
