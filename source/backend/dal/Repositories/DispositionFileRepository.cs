...
namespace Pims.Dal.Repositories
{
    ...
    public class DispositionFileRepository : BaseRepository<PimsDispositionFile>, IDispositionFileRepository
    {
        ...
        /// <returns></returns>
        public Paged<PimsDispositionFile> GetPageDeep(DispositionFilter filter, UserContextModel userContext)
        {
            using var scope = Logger.QueryScope();
            ...
            var query = GetCommonDispositionFileQueryDeep(filter, userContext);
        ...
        public List<PimsDispositionFileTeam> GetTeamMembers(UserContextModel userContext)
        {
            var predicate = PredicateBuilder.New<PimsDispositionFileTeam>(dt => true);

            if (userContext.IsContractor)
            {
                predicate
                    .And(dt => userContext.Regions.Contains(dt.DispositionFile.RegionCode))
                    .And(dt => dt.DispositionFile.PimsDispositionFileTeams.Any(p => p.PersonId == userContext.PersonId));
            }
        ...
        /// <param name="userContext">The calling user context.</param>
        /// <returns></returns>
        public List<PimsDispositionFile> GetDispositionFileExportDeep(DispositionFilter filter, UserContextModel userContext)
        {
            ...
            return GetCommonDispositionFileQueryDeep(filter, userContext).ToList();
        }
        ...
        /// <param name="userContext">The calling user context.</param>
        /// <returns></returns>
        private IQueryable<PimsDispositionFile> GetCommonDispositionFileQueryDeep(DispositionFilter filter, UserContextModel userContext)
        {
            ...
            {
                predicate = predicate
                    .And(disp => userContext.Regions.Contains(disp.RegionCode))
                    .And(disp => disp.PimsDispositionFileTeams.Any(x => x.PersonId == userContext.PersonId));
            }
            ...
            if (!string.IsNullOrWhiteSpace(filter.TeamMemberProfileTypeCode) && filter.TeamMemberPersonId.HasValue)
            {
                predicate = predicate.And(disp => disp.PimsDispositionFileTeams.Any(x => x.PersonId == filter.TeamMemberPersonId.Value
                                                                                && x.DspFlTeamProfileTypeCode == filter.TeamMemberProfileTypeCode));
            }
            ...
            query = (filter.Sort?.Length > 0) ? query.OrderByProperty(true, filter.Sort) : query.OrderByDescending(disp => disp.AssignedDt ?? DateOnly.FromDateTime(DateTime.MinValue));
