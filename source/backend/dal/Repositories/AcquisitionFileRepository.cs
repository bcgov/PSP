...
namespace Pims.Dal.Repositories
{
    ...
    public class AcquisitionFileRepository : BaseRepository<PimsAcquisitionFile>, IAcquisitionFileRepository
    {
        ...
        /// <summary>
        /// Get an Acquisition Files Export based on Filter.
        /// </summary>
        /// <param name="filter"></param>
        /// <param name="regions"></param>
        /// <param name="filterPersonId"></param>
        /// <returns>List of AcquisitionFileExportDto.</returns>
        public List<AcquisitionFileExportDto> GetAcquisitionFileExport(AcquisitionFilter filter, HashSet<short> regions, long? filterPersonId = null)
        {
            // RECOMMENDED - use a log scope to group all potential SQL statements generated by EF for this method call
            using var scope = Logger.QueryScope();
            ...
            var querySearch = from acqFile in Context.PimsAcquisitionFiles.Where(predicate)
                                ...
                                .Include(tm => tm.PimsAcquisitionFilePeople).ThenInclude(c => c.Person)
                                .Include(ow => ow.PimsAcquisitionOwners)
                              join fileProp in Context.PimsPropertyAcquisitionFiles
                                on acqFile.AcquisitionFileId equals fileProp.AcquisitionFileId into filePropGJ
                              from fp in filePropGJ.DefaultIfEmpty()
                              join property in Context.PimsProperties
                                .Include(a => a.Address)
                                    .ThenInclude(pr => pr.ProvinceState)
                                .Include(a => a.Address)
                                    .ThenInclude(pr => pr.Country)
                                on fp.PropertyId equals property.PropertyId into propGJ
                              from prop in propGJ.DefaultIfEmpty()
                              ...
                              select new AcquisitionFileExportDto()
                              {
                                  ...
                                  MinistryProject = acqFile.Project != null ? $"{acqFile.Project.Code} {acqFile.Project.Description}" : string.Empty,
                                  CivicAddress = (prop != null && prop.Address != null) ? prop.Address.FormatFullAddressString() : string.Empty,
            ...
            {
                throw new ExportHasNoDataException("Requested export has no data based on the criteria provided");
            }
