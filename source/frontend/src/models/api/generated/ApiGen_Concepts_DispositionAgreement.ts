/**
 * File autogenerated by TsGenerator.
 * Do not manually modify, changes made to this file will be lost when this file is regenerated.
 */
import { UtcIsoDate } from '@/models/api/UtcIsoDateTime';
import { UtcIsoDateTime } from '@/models/api/UtcIsoDateTime';

import { ApiGen_Base_BaseConcurrent } from './ApiGen_Base_BaseConcurrent';
import { ApiGen_Base_CodeType } from './ApiGen_Base_CodeType';

// LINK: @backend/apimodels/Models/Concepts/DispositionFile/DispositionAgreementModel.cs
export interface ApiGen_Concepts_DispositionAgreement extends ApiGen_Base_BaseConcurrent {
  agreementId: number;
  fileId: number;
  agreementType: ApiGen_Base_CodeType<string> | null;
  agreementStatusType: ApiGen_Base_CodeType<string> | null;
  agreementDate: UtcIsoDate | null;
  completionDate: UtcIsoDate | null;
  terminationDate: UtcIsoDate | null;
  commencementDate: UtcIsoDate | null;
  possessionDate: UtcIsoDate | null;
  depositAmount: number | null;
  noLaterThanDays: number | null;
  purchasePrice: number | null;
  legalSurveyPlanNum: string | null;
  offerDate: UtcIsoDate | null;
  expiryDateTime: UtcIsoDateTime | null;
  signedDate: UtcIsoDate | null;
  inspectionDate: UtcIsoDate | null;
  cancellationNote: string | null;
}
