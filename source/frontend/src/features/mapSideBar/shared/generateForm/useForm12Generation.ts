...
export interface IGeneratingModal {
  display: boolean;
  title?: string;
}
...
export function useForm12Generation(
  ...
  setIsGenerating: React.Dispatch<React.SetStateAction<IGeneratingModal>>,
) {
  ...
  const onGenerate = useCallback(
    (properties: ApiGen_Concepts_FileProperty[]) => {
      setIsGenerating({ display: true });
      generateForm12(properties.map(fp => fp.property)).finally(() =>
