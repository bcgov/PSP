...
import { useGenerateExpropriationForm7 } from './useGenerateExpropriationForm7';
...
const setup = (params?: {
  ...
  const { result } = renderHook(useGenerateExpropriationForm7, {
    wrapper: getWrapper(getStore(params?.storeValues)),
  });
  return result.current;
};

describe('useGenerateExpropriationForm7 functions', () => {
  beforeEach(() => {
  ...
  it('makes requests to expected api endpoints', async () => {
    ...
    await act(async () => generate(1, new ExpropriationForm7Model()));
  ...
  it('makes requests to expected api endpoints when expropriation authority is provided', async () => {
    const expropFormValues = new ExpropriationForm7Model();
  ...
  it('throws an error if no acquisition file is found', async () => {
    ...
    await expect(generate(1, new ExpropriationForm7Model())).rejects.toThrow(
