...
const FeeDeterminationSubForm: React.FunctionComponent<IFeeDeterminationSubFormProps> = ({
  ...
  return (
    <Section header="Fee Determination">
      <SectionField label="Public benefit" labelWidth={{ xs: 3 }} contentWidth={{ xs: 'auto' }}>
        <InlineYesNoSelect field="isPublicBenefit" />
      </SectionField>

      <SectionField label="Financial gain" labelWidth={{ xs: 3 }} contentWidth={{ xs: 'auto' }}>
          ...
          <StyledHelpText>
            ...
            {(fee === SuggestedFeeCode.LAF || fee === SuggestedFeeCode.ANY) && (
              <p className="m-0">
                ...
                impact to the land, the length of the LOO, and if MOTT requires a legal review.
              </p>
            )}
          </StyledHelpText>
        </span>
