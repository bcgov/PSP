...
export function createTableColumns(
  ...
  onShowPropertyInfo: (note: IProperty, inNewTab: boolean) => void,
): ColumnWithProps<IProperty>[] {
  const columns: ColumnWithProps<IProperty>[] = [
    {
      Header: 'PID',
      accessor: 'pid',
      align: 'right',
          ...
          <StyledDiv>
            {hasClaim(Claims.PROPERTY_VIEW) && (
              <StyledIconButton
                ...
                variant="light"
                onClick={() => {
                  onShowPropertyInfo(cellProps.row.original, false);
                }}
              >
                <FaEye />
              </StyledIconButton>
            ...
            {hasClaim(Claims.PROPERTY_VIEW) && (
              <StyledIconButton
                test-id="view-prop-ext"
                title="View Property"
                variant="light"
                onClick={() => {
                  onShowPropertyInfo(cellProps.row.original, true);
                }}
              >
                <FaExternalLinkAlt />
              </StyledIconButton>
