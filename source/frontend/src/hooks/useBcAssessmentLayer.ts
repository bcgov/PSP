...
export const useBcAssessmentLayer = (
  ...
  const getLegalDescriptionsWrapper = useWfsLayer<FeatureCollection>(
    url,
    {
  ...
  const getAddressesWrapper = useWfsLayer<FeatureCollection>(url, {
    name: names['ADDRESS'],
    withCredentials: true,
  });
  const getValuesWrapper = useWfsLayer<FeatureCollection>(url, {
    name: names['VALUE'],
    withCredentials: true,
  });
  const getChargesWrapper = useWfsLayer<FeatureCollection>(url, {
    name: names['CHARGE'],
    withCredentials: true,
  });
  const getFolioDescriptionsWrapper = useWfsLayer<FeatureCollection>(url, {
    name: names['FOLIO_DESCRIPTION'],
    withCredentials: true,
  });
  const getSalesWrapper = useWfsLayer<FeatureCollection>(url, {
    name: names['SALE'],
    withCredentials: true,
  });
...
export interface IBcAssessmentLegalDescription {
...
export interface IBcAssessmentFolioDescription {
  BCA_FD_SYSID: number;
  ROLL_NUMBER: string;
  FOLIO_ID: string;
  FOLIO_STATUS: string;
...
export interface IBcAssessmentAddress {
...
export interface IBcAssessmentSales {
  BCA_FS_SYSID: number;
  ROLL_NUMBER: string;
  FOLIO_ID: string;
  FOLIO_STATUS: string;
...
export interface IBcAssessmentValue {
...
export interface IBcAssessmentCharacteristic {
  SE_ANNO_CAD_DATA: string;
...
export interface IBcAssessmentSummary {
  FOLIO_DESCRIPTION: Partial<IBcAssessmentFolioDescription>;
  LEGAL_DESCRIPTION: Partial<IBcAssessmentLegalDescription>;
  ADDRESSES: Partial<IBcAssessmentAddress>[];
  SALES: Partial<IBcAssessmentSales>[];
  VALUES: Partial<IBcAssessmentValue>[];
  CHARACTERISTICS: Partial<IBcAssessmentCharacteristic>[];
}
