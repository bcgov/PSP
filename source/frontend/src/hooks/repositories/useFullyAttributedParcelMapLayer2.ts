...
import { PMBC_FullyAttributed_Feature_Properties2 } from 'models/layers/parcelMapBC';
 ...
 */
export const useFullyAttributedParcelMapLayer2 = (url: string, name: string) => {
  ...
  const findByLegalDescription = useCallback(
    async (legalDesc: string) => {
      ...
      return data as FeatureCollection<Geometry, PMBC_FullyAttributed_Feature_Properties2>;
    },
    [getAllFeatures],
  );
