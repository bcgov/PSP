...
class LeaseLicence {
  ...
  async createMinimumLease(lease) {
    ...
    await this.page.locator("#input-paymentReceivableTypeCode").selectOption({label: lease.AccountType})

    await this.page.locator("#input-regionId").selectOption({label: lease.MOTIRegion});
    await this.page.locator("#input-programTypeCode").selectOption({label: lease.Program});
    await this.page.locator("#input-leaseTypeCode").selectOption({label: lease.lease.AdminType});

    const purposeInput = await this.page.locator("#multiselect-purposes_input");
    await purposeInput.click();

    const purposesOptions = await this.page.locator("//input[@id='multiselect-purposes_input']/parent::div/following-sibling::div/ul[@class='optionContainer']");
    await purposesOptions.waitFor({status: "visible"});
    await lease.LeasePurpose.forEach(purpose => {
      const selectedOption = this.page.locator(purposesOptions).locator("li", {hasText: purpose});
