...
class ManagementFileDetails {
  ...
  async managementMainMenu() {
    await this.page.locator('div[data-testid="nav-tooltip-management"] a').click();
  }

  async createManagementFileLink() {
    await this.page.locator("//a[contains(text(), 'Create Management File')]").click();
  }

  async navigateManagementFileListView() {
    await this.page.locator("//a[contains(text(), 'Manage Management File')]").click();
  }

  async navigateManagementFileListActivities() {
    await this.page.locator("//a[contains(text(), 'Manage Management Activities')]").click();
  }

  async validateManagementFileDetailsPage() {
    await this.page.getByRole('h1', { name: 'Create Management File' }).isVisible();
  ...
  async createMinimumManagementFileDetails() {
    ...
    await this.page.locator('#input-purposeTypeCode').selectOption({ label: 'PIN' });
  }

  async updateManagementFileDetails() {
    // Status
    await expect(page.locator('label[for="managementFileStatusInput"]')).toBeVisible();
    if (mgmtFile.ManagementStatus !== "") {
        await page.locator('#managementFileStatusInput').selectOption({ label: mgmtFile.ManagementStatus });
    }

    // Project
    await expect(page.locator('label[for="managementFileProjectInput"]')).toBeVisible();
    ...
    // Product
    await expect(page.locator('label[for="managementFileProjectProductSelect"]')).toBeVisible();
    if (mgmtFile.ManagementMinistryProduct !== "") {
        await page.locator('#managementFileProjectProductSelect').selectOption({ label: mgmtFile.ManagementMinistryProduct });
    }

    // Funding
    await expect(page.locator('label[for="managementFileProjectFundingInput"]')).toBeVisible();
    if (mgmtFile.ManagementMinistryFunding !== "") {
        await page.locator('#managementFileProjectFundingInput').selectOption({ label: mgmtFile.ManagementMinistryFunding });
    }

    // MANAGEMENT DETAILS
    // File Name
    await expect(page.locator('label[for="managementFileNameInput"]')).toBeVisible();
    if (mgmtFile.ManagementName !== "") {
        await page.locator('#managementFileNameInput').fill(mgmtFile.ManagementName);
    }

    // Historical File Number
    await expect(page.locator('label[for="managementFileHistoricalFileInput"]')).toBeVisible();
    if (mgmtFile.ManagementHistoricalFile !== "") {
        await page.locator('#managementFileHistoricalFileInput').fill(mgmtFile.ManagementHistoricalFile);
    }

    // Purpose
    await expect(page.locator('label[for="managementFilePurposeSelect"]')).toBeVisible();
    if (mgmtFile.ManagementPurpose !== "") {
        await page.locator('#managementFilePurposeSelect').selectOption({ label: mgmtFile.ManagementPurpose });
    }

    // Additional Details
    await expect(page.locator('label[for="managementFileAdditionalDetailsInput"]')).toBeVisible();
    if (mgmtFile.ManagementAdditionalDetails !== "") {
        await page.locator('#managementFileAdditionalDetailsInput').fill(mgmtFile.ManagementAdditionalDetails);
    }

    // MANAGEMENT TEAM
    if (mgmtFile.ManagementTeam.length > 0) {
        // Delete all existing members
        while (await page.locator('.managementFileViewTeamMembersGroup').count() > 0) {
