...
class ManagementFileDetails {
  ...
  async saveManagementFile() {
    ...
    while (await this.managementFileConfirmationModal.count() > 0) {
      ...
      if (content.includes("The selected property already exists in the system's inventory")) {
        expect(header).toBe("User Override Required");
        expect(content).toContain("The selected property already exists in the system's inventory. However, the record is missing spatial details.");
        expect(content).toContain("To add the property, the spatial details for this property will need to be updated...");
        await this.sharedModals.mainModalClickOKBttn();

      } else if (content.includes("You have made changes to the properties in this file.")) {
        expect(header).toBe("Confirm changes");
        expect(content).toContain("You have made changes to the properties in this file.");
        ...
        const paragraph1 = await this.sharedModals.confirmationModalParagraph1();
        const paragraph2 = await this.sharedModals.confirmationModalParagraph2();
        expect(paragraph1).toContain("If you save it, only the administrator can turn it back on...");
  ...
  async validateManagementFileDetailsPage() {
    ...
    await expect(this.page.locator("label", { hasText: "Ministry project"})).toBeVisible();
    await expect(this.page.getByTestId("typeahead-project")).toBeVisible();
    await expect(this.page.locator("label", { hasText: "Funding" })).toBeVisible();
    ...
    //Management Details
    await expect(this.page.locator('h2:has-text("Management Details")')).toBeVisible();
  ...
  async validateManagementUpdateForm() {
    ...
    // Title
    const updateManagementTitle = await this.page.getByTestId("form-title").textContent();
    expect(updateManagementTitle).toEqual('Update Management File');

    // Status
    await expect(this.page.locator("label", { hasText: "Status" })).toBeVisible();
    await expect(page.locator("#input-fileStatusTypeCode")).toBeVisible();

    // Project
    await expect(page.locator('h2:has-text("Project")')).toBeVisible();
    await expect(page.locator("label", { hasText: "Ministry project" })).toBeVisible();
    ...
    // Management Details
    await expect(page.locator('h2:has-text("Management Details")')).toBeVisible();
    await expect(page.locator("label", { hasText: "File name" })).toBeVisible();
    await expect(page.locator("#input-fileName")).toBeVisible();
    await expect(page.locator("label", { hasText: "Historical file number" })).toBeVisible();
    ...
    await expect(page.locator("label", { hasText: "Additional details" })).toBeVisible();
  ...
  {
    const mgmtFile = await this.sharedFileProperties.getManagementFileProperties();
    expect(this.page.locator("h1", { hasText: "Management File" })).toBeVisible();
    ...
    expect(this.page.locator("label", { hasText: "Ministry project" })).toBeVisible();
    const actualProject = (await this.page.getByTestId('management-project').textContent())?.trim() || '';
    ...
    const actualProduct = (await this.page.getByTestId('management-product').textContent())?.trim() || '';
    ...
    const actualFunding = (await this.page.getByTestId('management-funding').textContent())?.trim() || '';
    const expectedFunding = data.ManagementMinistryFunding?.trim() || '';
    expect(actualFunding).toEqual(expectedFunding);

    //Management Details
    expect(this.page.locator("label", { hasText: "Management file name" })).toBeVisible();
    const actualName = (await this.page.getByTestId('management-file-name').textContent())?.trim() || '';
    const expectedName = data.ManagementName?.trim() || '';
    expect(actualName).toEqual(expectedName);

    expect(this.page.locator("label", { hasText: "Historical file number" })).toBeVisible();
    const actualHistorical = (await this.page.getByTestId('management-legacy-file-number').textContent())?.trim() || '';
    ...
    const actualPurpose = (await this.page.getByTestId('management-purpose').textContent())?.trim() || '';
    const expectedPurpose = data.ManagementPurpose?.trim() || '';
    expect(actualPurpose).toEqual(expectedPurpose);

    expect(this.page.locator("label", { hasText: "Additional details" })).toBeVisible();
    const actualDetails = (await this.page.getByTestId('management-additional-details').textContent())?.trim() || '';
