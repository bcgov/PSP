...
class SharedActivities {
  constructor(page) {
    ...
    async insertNewPropertyActivity(activity) {
        ...
        await this.page.locator("#input-activityTypeCode").selectOption({label: activity.PropertyActivityType});

        if (await this.page.locator("div[id='multiselect-activitySubtypeCodes'] i[class='custom-close']").count() > 0) {
            while (await this.page.locator("div[id='multiselect-activitySubtypeCodes'] i[class='custom-close']").count() > 0) {
                await this.page.locator("div[id='multiselect-activitySubtypeCodes'] i[class='custom-close']").first().click();
            }
        }
        ...
        if (activity.PropertyActivitySubTypeList.count() > 0)
        {
            activity.PropertyActivitySubTypeList.foreach(async (subType) => {
                ...
                await this.page.locator("div[id='multiselect-activitySubtypeCodes'] ul li").filter({ hasText: subType }).click();
            });
        }

        await this.page.locator("#input-activityStatusCode").selectOption({label: activity.PropertyActivityStatus});

        //Inserting Requested Added Date
        await this.page.locator("#datepicker-requestedCommenceDate").fill('');
        await this.page.locator("#datepicker-requestedCommenceDate").fill(activity.PropertyActivityRequestedCommenceDate);
        ...
        {
            await this.page.locator("#datepicker-completionDate").fill(activity.PropertyActivityCompletionDate);
            await this.page.locator("#datepicker-completionDate").press('Enter');
        }

        //Inserting Description
        await this.page.locator("#input-description").fill('');
        await this.page.locator("#input-description").fill(activity.PropertyActivityDescription);

        //Deleting previous Ministry Contacts and adding new
        while (await this.page.locator("//label[contains(text(),'Ministry contacts')]/parent::div/following-sibling::div/div/div/button").count() > 0) {
            await this.page.locator("//label[contains(text(),'Ministry contacts')]/parent::div/following-sibling::div/div/div/button").first().click();
        }

        if (activity.PropertyActivityMinistryContactList.count() > 0)
        {
            await this.page.locator("#input-ministryContact").click();
            this.sharedSelectContacts.selectContact(activity.PropertyActivityMinistryContactList[0], "");

            if (activity.PropertyActivityMinistryContactList.count() > 1)
            {
                activity.PropertyActivityMinistryContactList.forEach((ministryContact, index) => {
        ...
        if (activity.PropertyActivityRequestorContactMngr != null) {
            ...
            this.page.locator("#input-requestedSource").fill(activity.PropertyActivityRequestorContactMngr);
        }

        //Deleting Involved parties and adding new
        while (await this.page.locator("//label[contains(text(),'Involved parties')]/parent::div/following-sibling::div/div/div/button").count() > 0) {
            await this.page.locator("//label[contains(text(),'Involved parties')]/parent::div/following-sibling::div/div/div/button").first().click();
        }

        if (activity.PropertyActivityInvolvedPartiesExtContactsList.count() > 0)
        {
            await this.page.locator("//input[@id='input-involvedParties[0].id']/parent::div/parent::div/following-sibling::div/button").click();
            await this.sharedSelectContacts.selectContact(activity.PropertyActivityInvolvedPartiesExtContactsList[0], "");

            if (activity.PropertyActivityInvolvedPartiesExtContactsList.count() > 1)
            {
                activity.PropertyActivityInvolvedPartiesExtContactsList.forEach((involvedContact, index) => {
