...
class AdminTools {
  ...
  async verifyManageUsersListView() {
    ...
    await expect(this.page.getByRole("div").filter({ hasText: "Search By:" })).toBeVisible();
    await expect(this.page.locator("#input-role")).toBeVisible();
    await expect(this.page.locator("#input-region")).toBeVisible();
    await expect(this.page.locator("#input-businessIdentifierValue")).toBeVisible();
    ...
    await expect(this.page.getByRole("span").filter({ hasText: " users only" })).toBeVisible();
    ...
    await expect(
      this.page.getByTestId("sort-column-position")
    ).toBeVisible();
    await expect(
  ...
  async verifyUserRequestsListView() {
    ...
    await expect(
      this.page.locator("//label[text()='Search:']")
    ).toBeVisible();
    await expect(this.page.getByTitle("Search by IDIR/Last Name")).toBeVisible();
    await expect(this.page.locator("#search-button")).toBeVisible();
    await expect(this.page.locator("#reset-button")).toBeVisible();
  ...
  async verifyCDOGSUploadPage() {
    await expect(
      this.page.locator("//h1/span[contains(text(),'PIMS Document Template Management')]")
    ).toBeVisible();
    await expect(
      this.page.locator("//div[contains(text(),'Form Type:')]")
    ).toBeVisible();

    const cdogsTemplateSelect = await this.page.locator("select[class='form-select form-control']");
    expect(
      cdogsTemplateSelect
    ).toBeVisible();
    ...
    await cdogsTemplateSelect.selectOption({label: "Notice of Expropriation (Form 1)"});

    await expect(
      this.page.locator("//h2/div/div/div/div/span[contains(text(),'Documents')]")
    ).toBeVisible();
    await expect(
      this.page.locator("h2 button").first()
    ).toBeVisible();
    await expect(
      this.page.getByTestId("refresh-button").first()
    ).toBeVisible();

    await expect(this.page.getByRole("label").filter({hasText: "Filter by:"})).toBeVisible();
    ...
    await expect(this.page.getByTestId("sort-column-documentType")).toBeVisible();
    await expect(
  ...
  async verifyFinancialCodesListView() {
    ...
    await expect(this.page.locator("#input-codeValueOrDescription")).toBeVisible();
    await expect(this.page.locator("#input-showExpiredCodes")).toBeVisible();
    await expect(this.page.getByRole("span").filter({ hasText: "Show expired codes" })).toBeVisible();
    await expect(this.page.locator("#search-button")).toBeVisible();
    await expect(this.page.locator("#reset-button")).toBeVisible();
    ...
    await expect(
      this.page.getByTestId("sort-column-code")
    ).toBeVisible();
    await expect(
    ...
    await expect(
      this.page.getByTestId("sort-column-expiryDate")
    ).toBeVisible();
  ...
  async verifyFinancialCodeCreateForm() {
    ...
    expect(await this.page.getByRole("h1").filter({hasText: "Create Financial Code"})).toBeVisible();
    expect(await this.page.getByRole("label").filter({hasText: "Code type"})).toBeVisible();
    ...
    expect(await this.page.getByRole("label").filter({hasText: "Code value"})).toBeVisible();
    expect(await this.page.locator("#input-code")).toBeVisible();

    expect(await this.page.getByRole("label").filter({hasText: "Code description"})).toBeVisible();
    expect(await this.page.locator("#input-description")).toBeVisible();

    expect(await this.page.getByRole("label").filter({hasText: "Effective date"})).toBeVisible();
    expect(await this.page.locator("#datepicker-effectiveDate")).toBeVisible();

    expect(await this.page.getByRole("label").filter({hasText: "Expiry date"})).toBeVisible();
    expect(await this.page.locator("#datepicker-expiryDate")).toBeVisible();

    expect(await this.page.getByRole("label").filter({hasText: "Display order"})).toBeVisible();
    ...
    await expect(this.page.locator("//div[text()='Cancel']:parent::button")).toBeVisible();
    await expect(this.page.locator("//div[text()='Save']:parent::button")).toBeVisible();
  }
  ...
  async verifyBCFTAPage() {
    ...
    await expect(
      this.page.getByRole("div").filter({hasText: 'Upload a csv file, that contains the list of all PIDs currently owned by BCTFA, as provided by LTSA. Uploading this file here will update the BCTFA ownership layer within PIMS to reflect the PIDS listed in the uploaded file.'})
    ).toBeVisible();
    await expect(this.page.getByRole("div").filter({ hasText: "Drag files here to attach or" })).toBeVisible();
    await expect(this.page.getByRole("label").filter({ hasText: "Browse" })).toBeVisible();
    await expect(
      this.page.getByTestId("upload-input")
    ).toBeVisible();
  }
}
